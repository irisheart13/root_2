<?php
    ...
    $tbl_tbl_fileUpload_query = $conn->prepare("SELECT date_of_submission, title, main_author, co_author_1, co_author_2, others, file_upload, research_status
                                        FROM tbl_fileUploadWHERE username = ? ORDER BY date_of_submission DESC LIMIT ? OFFSET ?");
    $tbl_tbl_fileUpload_query->bind_param("sii", $user_name, $limit, $offset);
    $tbl_tbl_fileUpload_query->execute();
    $tbl_tbl_fileUpload_query->store_result();
    $tbl_tbl_fileUpload_query->bind_result($date_of_submission, $title, $main_author, $co_author_1, $co_author_2, $others, $file_upload, $research_status);

    // Get total records for pagination
    $total_query = $conn->prepare("SELECT COUNT(*) FROM tbl_fileUploadWHERE username = ?");
                        ...
                        <div class="row">
                            <div class="row"> <!--File Attachment START-->
                                ...
                                <div class="col-12 col-md-8 p-1">
                                    <input type="file" name="file" id="tbl_tbl_fileUpload" class="form-control custom-input" required>
                                </div>
            ...
            <table class="table table-striped custom-table">
                ...
                <tbody>
                    <?php
                        ...
                        if ($tbl_tbl_fileUpload_query->num_rows > 0) {
                            while ($tbl_tbl_fileUpload_query->fetch()) {
                                echo "<tr>";
                        ...
                        $tbl_tbl_fileUpload_query->close();
                    ?>
                </tbody>
            </table>
            <!-- Pagination Links -->
