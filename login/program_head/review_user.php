...
if (!$id || !in_array($type, ['research', 'abstract'])) {
    die("Invalid request.");
}

// Fetch file details including uploader's program
$query = $conn->prepare("
    SELECT file_research_paper, file_abstract, program 
    FROM tbl_fileUpload 
    WHERE id = ?
");
...
$query->execute();
$query->store_result();
$query->bind_result($file_research_paper, $file_abstract, $uploader_program);
$query->fetch();
$query->close();

// Validate retrieved data
if (!$file_research_paper && !$file_abstract) {
    die("No file found for the given ID.");
}

// Construct file path dynamically
$base_dir = "../general_user/uploadedFile/$department/$uploader_program";
